...
### Fixed
- **Multi-file submissions no longer crash at DAG build with `No signature of method: java.util.ArrayList.getFileSystem()`.** WES serializes a multi-file FedImpute submission as a JSON array, so `params.input` arrived as a `java.util.ArrayList`. The string-only branches in `validateInputFiles()`/`createInputChannel()` and `bin/process_input.py` mis-routed it: `List.contains(',')` tests element membership (not substring), so the list fell through to the single-file branch where `file([...])` threw. Any liftover job with more than one input file (e.g. 22 per-chromosome VCFs) failed before any task ran; only single-file jobs through the legacy string path worked. `main.nf` now normalizes a `List` input to the comma-separated form the pipeline already supports (`params.input.join(',')`) before validation and before handing it to `LIFTOVER_WORKFLOW`/`INPUT_HANDLER`; a plain string passes through unchanged.
